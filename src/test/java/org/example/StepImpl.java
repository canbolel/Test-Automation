...
import com.turkishairlinesmobile.helper.RandomString;
...
//        tapElementWithCoordinate(me.getLocation().x, me.getLocation().y);
    ...
    @Step("Choose Gender of <index> person")
    public void selectGender(int index) {
        if (passengers.get(index).getPassengerGender().equals("Male")) {
            String gender = "Mr.";
            try {
    ...
    public void sendToElement(String key, int index) {
        switch (key) {
            case "txtName":
                sendKeysByKey(key, passengers.get(index).getPassengerName());
                break;
            case "txtSurname":
                sendKeysByKey(key, passengers.get(index).getPassengerLastName());
                break;
            case "year":
            case "month":
            case "day":
                break;
            case "txtEmail":
                sendKeysByKey(key, passengers.get(index).getPassengerEmail());
                break;
            case "tcNo":
                sendKeysByKey(key, passengers.get(index).getPassengerTc());
                break;
            case "phoneNumber":
                sendKeysByKey(key, passengers.get(index).getPassengerPhone());
                break;
            case "airportSearch-from":
                key = key.split("-")[0];
                sendKeysByKey(key, routes.get(0).getFrom());
                break;
            case "airportSearch-to":
                key = key.split("-")[0];
                sendKeysByKey(key, routes.get(0).getTo());
                break;
        }
    }
